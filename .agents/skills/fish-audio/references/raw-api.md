...
Fish Audio's official raw-API skill, kept verbatim except where testing against
the live API contradicted it. Every such change is marked **VERIFIED**.

It does **not** cover the conversational agent API (`/v1/agent/*`); that lives in
[`agent.md`](agent.md).
...
| — | `/v1/agent/*` | Conversational agent. Not covered here, see [`agent.md`](agent.md) |
...
- `model`: values `s1`, `s2-pro`, `s2.1-pro`, `s2.1-pro-free`, `drama-3-preview`. If omitted or unrecognized, the server falls back to `s2.1-pro` (paid). **VERIFIED**: `s2.1-pro-fre`, one character short of the free model, returns 402 with no hint that the name was wrong. Note the asymmetry — an unknown value in the **body** is silently ignored and succeeds, while an unknown **model header** is silently billed as paid. Default to `s2.1-pro` for production; use `s2.1-pro-free` for free-tier evaluation and prototyping (same model, no TTFA/DPA guarantees).
...
**VERIFIED: there is no free tier for ASR.** Every model header tried (`transcribe-1`, `transcribe-1-free`, `asr-1-free`, `s2.1-pro-free`, and none at all) returns 402 without paid credit. If the audio came from an agent conversation you already have its transcript for free — see [`agent.md`](agent.md).
...
Optional: `visibility` (`public` | `unlist` | `private`, **default `private`** — **VERIFIED**: creating a model with no `visibility` returns `private`, not `public` as originally documented. Set it explicitly regardless; a cloned voice made public puts someone's likeness on a public listing. `cover_image` is required if `public`), `description`, `cover_image`, `texts` (transcripts matching each voice; if omitted, ASR is run on the audio), `tags` (string or array), `enhance_audio_quality` (bool, default `false`).
...
**VERIFIED**: responses carry `ratelimit-limit-concurrency` and `ratelimit-current-concurrency`. The tested account caps at **5** concurrent requests. For anything running many sessions at once this ceiling binds well before per-call cost does.
