...
Fish Audio's official SDK skill, kept verbatim except where noted **VERIFIED**.

For the conversational agent the SDKs below are the wrong ones: use
`@fishaudio/agent-client` (or `@fishaudio/agent-react`) instead, covered in
[`agent.md`](agent.md).
...
> **The `references/` files this index points at were not shipped with the skill.**
> Every `references/*.md` link below is dead. Use [`raw-api.md`](raw-api.md) for the
> underlying HTTP behaviour, or read the SDK's own type definitions.
...
**These links are dead** (see above). [`raw-api.md`](raw-api.md) covers the same
ground at the HTTP level.
...
## Not covered by these SDKs

The conversational agent API has its own client packages, unrelated to `fishaudio` /
`fish-audio`:

```bash
pnpm add @fishaudio/agent-client   # framework-free
pnpm add @fishaudio/agent-react    # React, wraps the above
```
...
- Python `latency` accepts only **`"normal"` or `"balanced"`** (default `"balanced"`); there is no `"low"`.
  **VERIFIED**: the raw API *does* accept `latency: "low"`, and it matters — measured first audio at
  200-430ms on `low` versus ~900ms on `normal`. If you are building anything conversational, this SDK
  limitation costs you half a second per turn; call the endpoint directly instead.
